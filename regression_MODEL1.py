...
#Model 1(全部參數一次項)
parameter=18*9+1  #總共18測項*9hr+1截距
...
for u in range(3):
    ...
    x_train=np.concatenate((Nfolds_cross_validation_x[Nfolds[u][0]],Nfolds_cross_validation_x[Nfolds[u][1]]), axis=0).astype(float)
    y_train=np.concatenate((Nfolds_cross_validation_y[Nfolds[u][0]],Nfolds_cross_validation_y[Nfolds[u][1]]), axis=0).astype(float)
    x_valid=Nfolds_cross_validation_x[Nfolds[u][2]]
    ...
    for i in range(iter_time):
        ...
        """
        if(i%100==0):
            loss = np.sqrt(np.sum(np.power(np.dot(x_set, parameter_set) - y_train, 2))/np.shape(x_train)[0])#rmse
            print(i,str(loss))
        """    
        adagrad += gradient ** 2
